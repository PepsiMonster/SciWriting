...
% Оформление (можешь менять тему и цветовую схему)
\usetheme{Warsaw}
...
\begin{frame}{Что такое 5G?}
  \begin{block}{Кратко}
    \begin{itemize}
      ...
      \item<2-> Используют более высокие частоты
        (например, около \(3{,}5\,\text{ГГц}\) и выше).
  ...
  \begin{block}{Роль базовых станций (вышек)}
    \begin{itemize}
...
\begin{frame}{Шум и отношение сигнал/шум}
  \begin{block}{Шум}
    \begin{itemize}
      \item<1-> Тепловой шум приёмника.
      \item<2-> Помехи от электроники, других устройств.
    \end{itemize}
  \end{block}

  \pause

  \begin{block}{Отношение сигнал/шум (SNR)}
...
\begin{frame}{Интерференция (взаимные помехи)}
  ...
  \begin{columns}
    \begin{column}{0.55\textwidth}
      ...
      \begin{block}{Эффект}
        \uncover<4->{Сумма сигналов может как усиливать, так и ослаблять
        полезный сигнал.}
        \uncover<5->{Это приводит к провалам уровня сигнала и ошибкам при приёме.}
      \end{block}
    \end{column}

    \begin{column}{0.45\textwidth}
      % Замените на свою картинку схемы сети 5G
      \begin{center}
        \uncover<2->{
...
\begin{frame}{Методы борьбы с шумом и помехами}
  \begin{enumerate}
    ...
    \item \uncover<2->{\alert{MIMO и beamforming} \\
      Использование нескольких антенн для формирования узких лучей
      в сторону конкретного пользователя.}
    \item \uncover<3->{\alert{Малые соты} \\
      Больше небольших базовых станций, меньшие расстояния, лучше
      \(\text{SNR}\).}
    \item \uncover<4->{\alert{Адаптивная модуляция и кодирование} \\
      При хорошем \(\text{SNR}\) --- высокая скорость, при плохом \(\text{SNR}\)
...
\begin{frame}{Итоги}
  \begin{block}{Главные идеи}
    \begin{itemize}
      \item<1-> 5G использует более высокие частоты, поэтому сигнал
        сильнее затухает и чувствителен к препятствиям.
      \item<2-> Качество связи определяется уровнем шума и интерференции
        (\(\text{SNR}\)).
      \item<3-> Для борьбы с проблемами применяются MIMO, beamforming,
        малые соты и умное управление ресурсами сети.
    \end{itemize}
  \end{block}
  ...
  \uncover<4->{
    \vspace{0.5cm}
    \centering
    \alert{Идея для доработки: добавить реальные цифры по скоростям и частотам
