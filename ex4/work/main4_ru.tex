...
\renewcommand{\includegraphics}[2][]{\origincludegraphics[#1]{image.png}}
...
\subsection{ Центрированное изображение (база)}
Ниже — центрированная картинка.
...
\subsection{ Высота против ширины}
...
\subsection{ Масштаб и поворот}
\begin{center}
\includegraphics[scale=0.6, angle=15]{example-image}
\end{center}

\subsection{ Обрезка и отсечение (trim + clip)}
...
\subsection{ Floats с разными спецификаторами размещения}
...
\subsection{ Принудительное размещение \texttt{[H]}}
...
\subsection{ \texttt{\textbackslash textwidth} против \texttt{\textbackslash linewidth}}
...
\subsection{ Что будет, если \texttt{\textbackslash label} стоит до \texttt{\textbackslash caption}?}
...
\subsection{ Метка у формулы: внутри окружения и после него}
\begin{equation}
e^{i\pi}+1 = 0
\label{eq:inside}
\end{equation}
